...
export function EmailCapture() {
  ...
  return (
    <section id="contacto" className="py-20 bg-navy-950/40 border-t border-navy-800/60 relative overflow-hidden">
      {/* Ambient blob */}
      <div className="blob w-[600px] h-[400px] bg-amber-500/5 top-0 right-0" />
          ...
          <div className="bg-navy-900 border border-navy-800 rounded-3xl p-6 sm:p-8 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                id="email-submit-btn"
                className="w-full mt-1 py-3.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-navy-950 font-bold text-sm uppercase tracking-wider rounded-xl shadow-lg shadow-amber-500/10 flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-0.5"
              >
                <span>Enviar</span>
...
/* ── Helper ── */
function FormField({ id, type, value, onChange, placeholder, required, icon, label }) {
  return (
    <div>
      {label && (
      ...
      <div className="relative">
        ...
        <input
          ...
          required={required}
          className="w-full bg-[#060911] border border-navy-800 focus:border-amber-500 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-colors duration-200"
        />
      </div>
    </div>
  );
